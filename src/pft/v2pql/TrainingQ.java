...
import plan.v2pql.V2PQLProcess_Backup;

/**
 *
 * @author Khiet
 */
public class TrainingQ {
    ...
    //init data
        ...
        for(int t = 0; t < time; t++){
                ...
                if(faultyPM.size()>0){
                    ...
                    //double[] epsilon = {0.1};
                    double gamma = 0.8, nguy = 0.1;

                    for(int i =0; i < epsilon.length; i++){
                        V2PQLProcess qlearningProcess = new V2PQLProcess(pms, 
                        vms, migrateVM, safePM,
                        epsilon[i], 
                        //epsilon[0], 
                        gamma, nguy);
                        qlearningProcess.run();
                    }
                }
