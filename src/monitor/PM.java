...
public class PM {
    ...
    public double getH(){
        ...
        double u_hat = (cpu+ram+disk)/3;
        return Math.sqrt(Math.pow(cpu/u_hat-1,2)+Math.pow(ram/u_hat-1,2)+Math.pow(disk/u_hat-1,2));
    }
    ...
    public boolean addableVM(VM vm) {
        if (ccpu - cpu*ccpu - vm.getCcpu() > 0
                && cram-ram*cram - vm.getCram() > 0
                && cdisk-disk*cdisk - vm.getCdisk() > 0) {
            return true;
        }
        return false;
    }
